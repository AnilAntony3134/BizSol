...
export const editUser = (id, formData, history) => async (dispatch, getState) => {
  ...
  try {
    ...
    const response = await axios.put(`https://localhost:80/api/users/${id}`, formData, options);
...
export const getProfile = (username, history) => async (dispatch, getState) => {
  ...
  try {
    const options = attachTokenToHeaders(getState);
    const response = await axios.get(`https://localhost:80/api/users/${username}`, options);
...
export const deleteUser = (id, history) => async (dispatch, getState) => {
  ...
  try {
    const options = attachTokenToHeaders(getState);
    const response = await axios.delete(`https://localhost:80/api/users/${id}`, options);
